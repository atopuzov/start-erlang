...
mock_io() ->
    %% For this one we mock the IO system so that instead of printing messages
    %% and getting input to and from the user, we instead have a message-
    %% passing interface that will be inspectable.
    %%
    %% Note that because the `io` module is pre-compiled by the VM, we have to
    %% 'unstick' it first, and be careful to keep it mocked as little as
    %% possible.
    Parent = self(),
...
%% Pressing a given key through the message-passing interface will yield
%% expected output. There should be a prompt waiting for a key.
